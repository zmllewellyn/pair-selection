...
companies = ['AEE', 'PXD', 'WMB', 'DTE', 'PPL', 'ETR', 'EIX', 'FE', 'PEG', 'ES', 'ED', 'OKE', 'WEC', 'VLO', 'MPC', 'XEL', 'OXY', 'PSX', 'EOG', 'SRE', 'EXC', 'KMI', 'AEP', 'COP', 'DUK', 'D', 'SO', 'NEE', 'CVX', 'XOM']

# Create a dataframe using log return time series data
df_data = {}
for comp in companies:
    # Open the companies csv file
    file = open("cold_storage\\log_return_csv\\" + comp + "_log_return.csv")
...
for comp in company:
    # Open the companies csv file
    file = open("cold_storage\\clean_csv\\" + comp + "_clean.csv")
    value = list(csv.reader(file))
...
for pair in pair_spread:
    ...
    # Mean reverting/anti-persisten behaviour when H<0.5
    if H < 0.5:
...
print(len(pair_spread))
print(len(pair_hurst))
print(len(pair_coint))
print(pair_coint)
...
for pair in pair_coint:
    ...
    print("Graph of " + pair[0] + " and " + pair[1])
    plt.plot(pair[2])
    plt.plot(m_data)
    plt.plot(sa_data)
    plt.plot(sb_data)
